...
try {
  ...
  try {
    ...
    const normalizedUrl = REDIS_URL.replace(/^rediss?:/i, isTls ? 'https:' : 'http:');
...
const DEFAULT_TTL_SECONDS = Number(process.env.REDIS_TTL_SECONDS) || 84600;
  ...
  if (cachedData !== null && cachedData !== undefined) {
    ...
    if (isStale) {
      ...
      revalidateBackground(cacheKey, firebaseSafeKey, fetchFn, ttlSeconds).catch(err => {
        console.warn(`[Cache] Stale revalidation warning for ${cacheKey}:`, err?.message || err);
      });
    }
...
async function saveToCache(cacheKey: string, firebaseSafeKey: string, data: any, ttlSeconds: number) {
  ...
  try {
    const firebaseRef = ref(db, `kryzox_cache/${firebaseSafeKey}`);
    // Firebase Realtime DB throws an error if any property in object tree is undefined.
    // JSON.stringify / JSON.parse strips all undefined properties cleanly.
    const firebaseSafePayload = JSON.parse(JSON.stringify(payload));
    await set(firebaseRef, firebaseSafePayload);
  } catch (e: any) {
    console.error(`[Cache] Firebase save failed for ${cacheKey}:`, e?.message || e);
  }
}
...
async function revalidateBackground(cacheKey: string, firebaseSafeKey: string, fetchFn: () => Promise<any>, ttlSeconds: number) {
  try {
    ...
    if (e?.status === 404 || e?.message?.includes('404')) {
      console.warn(`[Cache Revalidate Info] Key: ${cacheKey} upstream source returned 404`);
    } else {
      console.warn(`[Cache Revalidate Warning] Key: ${cacheKey} revalidation notice:`, e?.message || e);
    }
  }
}
