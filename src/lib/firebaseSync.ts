...
export function cleanForFirebase<T>(val: T): T {
...
export async function saveWatchProgressDb(email: string, progress: WatchProgress) {
  ...
  runWithTimeout(set(progressRef, cleanForFirebase(progress)), DB_TIMEOUT_MS, null);
}
...
export async function saveFavoriteDb(email: string, anime: Anime, isFavorite: boolean) {
  ...
  const dbTask = (async () => {
    if (isFavorite) {
      await set(favoriteRef, cleanForFirebase(anime));
    } else {
...
export async function saveBookmarkDb(email: string, anime: Anime, isBookmarked: boolean) {
  ...
  const dbTask = (async () => {
    if (isBookmarked) {
      await set(bookmarkRef, cleanForFirebase(anime));
    } else {
...
import { publishToTelegram } from './telegramApi';
import { sanitizeAnimeTitle } from './animeImportSystem';

export async function addCustomAnime(id: string, anime: any, options?: { skipTelegram?: boolean }) {
  const animeRef = ref(db, `animes/${id}`);
  
  // Always sanitize the title before saving to ensure clean database entries
  const cleanedTitle = anime.title ? sanitizeAnimeTitle(anime.title) : (anime.name ? sanitizeAnimeTitle(anime.name) : '');
  
  const animeToSave = {
    ...anime,
    id,
    title: cleanedTitle || anime.title || anime.name || 'Untitled Anime',
  };

  const cacheKey = "anova_custom_animes";
  const localAnimes = getLocalItem<Record<string, any>>(cacheKey, {});
  localAnimes[id] = animeToSave;
  setLocalItem(cacheKey, localAnimes);

  await runWithTimeout(set(animeRef, cleanForFirebase(animeToSave)), DB_TIMEOUT_MS, null);

  // Asynchronous non-blocking Telegram post trigger (Only if NOT already notified and NOT skipped)
  try {
    const shouldSkipTelegram = options?.skipTelegram || anime.skipTelegram || anime.telegramNotified === true;

    if (!shouldSkipTelegram) {
      const isMovie = String(animeToSave.type).toLowerCase() === 'movie' || 
                      String(animeToSave.format).toLowerCase() === 'movie' ||
                      animeToSave.categories?.movies === true ||
                      String(animeToSave.title || '').toLowerCase().includes('movie');
      
      const isHindiDub = animeToSave.categories?.['hindi-dubbed'] === true || 
                         animeToSave.hindiAvailable === true || 
                         String(animeToSave.title || '').toLowerCase().includes('hindi');
      
      const subOrDub = isHindiDub ? 'Hindi Dub' : (animeToSave.subOrDub || (animeToSave.dub ? 'Dub' : 'Sub'));

      // Mark telegramNotified flag on the saved anime record to prevent double posts
      const updatedRecord = { ...animeToSave, telegramNotified: true };
      setLocalItem(cacheKey, { ...localAnimes, [id]: updatedRecord });
      set(animeRef, cleanForFirebase(updatedRecord)).catch(() => {});

      publishToTelegram({
        type: isMovie ? 'movie' : 'anime',
        title: animeToSave.title,
        animeId: id,
        subOrDub,
        rating: animeToSave.rating || animeToSave.score || '8.5/10',
        genres: animeToSave.genres || animeToSave.genre || [],
        releaseDate: animeToSave.released || animeToSave.releaseYear || new Date().toISOString().split('T')[0],
        description: animeToSave.description || animeToSave.synopsis || '',
        poster: animeToSave.poster || animeToSave.image || '',
        banner: animeToSave.banner || animeToSave.poster || ''
      }).catch(() => {});
    }
  } catch (_) {}
}
...
export async function getCustomAnimes(): Promise<Record<string, any>> {
  ...
  const dbTask = (async () => {
    ...
    if (snap.exists()) {
      const val = snap.val();
      setLocalItem(cacheKey, val);
      return val;
    }
    return {};
  })();

  const finalAnimes = await runWithTimeout(dbTask, 6000, localAnimes);
  return finalAnimes;
}

export async function updateAnimeEpisodesCount(animeId: string) {
  try {
    const epRef = ref(db, `episodes/${animeId}`);
    const snap = await get(epRef);
    let count = 0;
    if (snap.exists()) {
      const eps = snap.val();
    ...
    if (animeSnap.exists()) {
      ...
      const cacheKey = "anova_custom_animes";
      const localAnimes = getLocalItem<Record<string, any>>(cacheKey, {});
      if (localAnimes[animeId]) {
        localAnimes[animeId].episodes = count;
        localAnimes[animeId].episodesCount = count;
        setLocalItem(cacheKey, localAnimes);
      }
    }
  } catch (err) {
    console.error(`[Firebase sync] Failed to auto-update episodes count for ${animeId}:`, err);
  }
}
...
export async function addCustomEpisode(animeId: string, episodeNumber: number, episode: any) {
  ...
  const seNum = Number(episode?.seasonNumber || 1);
  const seEpNum = Number(episode?.seasonEpisodeNumber || episodeNumber);
  const provEpId = episode?.providerEpisodeId || episode?.episodeId || `moviebox-${episode?.providerSeriesId || animeId}-S${seNum}-E${seEpNum}`;
  const provEpUrl = episode?.providerEpisodeUrl || episode?.url || episode?.videoSources?.sub?.url || '';

  const enrichedEpisode = {
    ...episode,
    id: episode?.id || `${animeId}-ep-${episodeNumber}`,
    seriesId: animeId,
    seasonId: episode?.seasonId || `${animeId}-season-${seNum}`,
    seasonNumber: seNum,
    seasonEpisodeNumber: seEpNum,
    episodeNumber: Number(episodeNumber),
    number: Number(episodeNumber),
    title: episode?.title || `Episode ${episodeNumber}`,
    provider: episode?.provider || (provEpUrl.includes('moviebox') || provEpUrl.includes('aoneroom') ? 'moviebox' : 'custom'),
    providerSeriesId: episode?.providerSeriesId || '',
    providerEpisodeId: provEpId,
    providerEpisodeUrl: provEpUrl,
    videoSources: episode?.videoSources || {},
    streamId: episode?.streamId || provEpUrl || '',
    updatedAt: Date.now(),
  ...
  localEps[episodeNumber] = enrichedEpisode;
  setLocalItem(cacheKey, localEps);

  await runWithTimeout(set(epRef, cleanForFirebase(enrichedEpisode)), DB_TIMEOUT_MS, null);
  await updateAnimeEpisodesCount(animeId);

  // Asynchronous non-blocking Telegram episode/movie post
  try {
    const animeRef = ref(db, `animes/${animeId}`);
    get(animeRef).then((animeSnap) => {
      const anime = animeSnap.exists() ? animeSnap.val() : {};
      const isMovie = String(anime.type).toLowerCase() === 'movie' || 
                      String(anime.format).toLowerCase() === 'movie' || 
                      anime.categories?.movies === true ||
                      String(anime.title || '').toLowerCase().includes('movie') ||
                      String(episode?.title || '').toLowerCase().includes('movie') ||
                      episode?.title === 'Full movie';

      const isHindiDub = episode?.videoSources?.hindi_dub?.enabled || 
                         anime.categories?.['hindi-dubbed'] === true || 
                         anime.hindiAvailable === true || 
                         String(episode?.title || '').toLowerCase().includes('hindi') ||
                         String(anime.title || '').toLowerCase().includes('hindi');

      const subOrDub = isHindiDub ? 'Hindi Dub' : (episode?.videoSources?.eng_dub?.enabled ? 'Eng Dub' : (episode?.dub ? 'Dub' : (anime.subOrDub || 'Sub')));

      const seriesTitle = anime.title || anime.name || episode.animeTitle || episode.title || 'Untitled Anime';
      const seriesPoster = anime.poster || anime.image || anime.coverImage || episode.thumbnail || episode.poster || '';
      const seriesBanner = anime.banner || anime.backdrop || anime.poster || anime.image || episode.banner || '';
      ...
      if (isMovie) {
        publishToTelegram({
          ...
          animeId,
          subOrDub,
          rating: anime.rating || anime.score || episode.rating || '8.5/10',
          genres: anime.genres || anime.genre || [],
          releaseDate: anime.released || anime.releaseYear || new Date().toISOString().split('T')[0],
          description: anime.description || anime.synopsis || episode.description || '',
          poster: seriesPoster,
          banner: seriesBanner
        }).catch(() => {});
      } else {
        publishToTelegram({
          ...
          episodeNumber,
          subOrDub,
          rating: episode.rating || anime.rating || '8.5/10',
          description: episode.description || episode.overview || anime.description || '',
          poster: seriesPoster,
          banner: seriesBanner
        }).catch(() => {});
      }
    }).catch(() => {
      const isMovie = String(episode?.title || '').toLowerCase().includes('movie') || episode?.title === 'Full movie';
      publishToTelegram({
        type: isMovie ? 'movie' : 'episode',
        title: episode.animeTitle || episode.title || `Episode ${episodeNumber}`,
        animeId,
        episodeNumber,
        subOrDub: episode?.videoSources?.hindi_dub?.enabled ? 'Hindi Dub' : (episode?.dub ? 'Dub' : 'Sub'),
        rating: episode?.rating || '8.5/10',
        description: episode?.description || episode?.overview || '',
        poster: episode?.thumbnail || episode?.poster || '',
...
export async function addCustomEpisodesBatch(animeId: string, episodesMap: Record<number, any>) {
  ...
  for (const [key, ep] of Object.entries(episodesMap)) {
    const epNum = Number(key);
    const seNum = Number(ep?.seasonNumber || 1);
    const seEpNum = Number(ep?.seasonEpisodeNumber || epNum);
    const provEpId = ep?.providerEpisodeId || ep?.episodeId || `moviebox-${ep?.providerSeriesId || animeId}-S${seNum}-E${seEpNum}`;
    const provEpUrl = ep?.providerEpisodeUrl || ep?.url || ep?.videoSources?.sub?.url || '';

    enrichedMap[epNum] = {
      ...ep,
      id: ep?.id || `${animeId}-ep-${epNum}`,
      seriesId: animeId,
      seasonId: ep?.seasonId || `${animeId}-season-${seNum}`,
      seasonNumber: seNum,
      seasonEpisodeNumber: seEpNum,
      episodeNumber: epNum,
      number: epNum,
      title: ep?.title || `Episode ${epNum}`,
      provider: ep?.provider || (provEpUrl.includes('moviebox') || provEpUrl.includes('aoneroom') ? 'moviebox' : 'custom'),
      providerSeriesId: ep?.providerSeriesId || '',
      providerEpisodeId: provEpId,
      providerEpisodeUrl: provEpUrl,
      videoSources: ep?.videoSources || {},
      streamId: ep?.streamId || provEpUrl || '',
      updatedAt: Date.now(),
  ...
  const cacheKey = `anova_custom_episodes_${animeId}`;
  const localEps = getLocalItem<Record<string, any>>(cacheKey, {});
  const mergedEps = { ...localEps, ...enrichedMap };
  setLocalItem(cacheKey, mergedEps);

  await runWithTimeout(update(episodesRef, cleanForFirebase(enrichedMap)), DB_TIMEOUT_MS * 3, null);
  await updateAnimeEpisodesCount(animeId);

  // Asynchronous non-blocking Telegram batch episode / movie post
  try {
    const keys = Object.keys(episodesMap);
    if (keys.length > 0) {
      const animeRef = ref(db, `animes/${animeId}`);
      get(animeRef).then((animeSnap) => {
        const anime = animeSnap.exists() ? animeSnap.val() : {};
        const firstEpKey = keys[0];
        const ep = episodesMap[Number(firstEpKey)];

        const isMovie = String(anime.type).toLowerCase() === 'movie' || 
                        String(anime.format).toLowerCase() === 'movie' || 
                        anime.categories?.movies === true ||
                        String(anime.title || '').toLowerCase().includes('movie') ||
                        String(ep?.title || '').toLowerCase().includes('movie') ||
                        ep?.title === 'Full movie';

        const isHindiDub = ep?.videoSources?.hindi_dub?.enabled || 
                           anime.categories?.['hindi-dubbed'] === true || 
                           anime.hindiAvailable === true || 
                           String(ep?.title || '').toLowerCase().includes('hindi') ||
                           String(anime.title || '').toLowerCase().includes('hindi');

        const subOrDub = isHindiDub ? 'Hindi Dub' : (ep?.videoSources?.eng_dub?.enabled ? 'Eng Dub' : (ep?.dub ? 'Dub' : (anime.subOrDub || 'Sub')));

        const seriesTitle = anime.title || anime.name || ep?.animeTitle || 'Untitled Anime';
        const seriesPoster = anime.poster || anime.image || anime.coverImage || ep?.thumbnail || ep?.poster || '';
        const seriesBanner = anime.banner || anime.backdrop || anime.poster || anime.image || ep?.banner || '';

        if (isMovie) {
          // If the movie notification was already sent via addCustomAnime, skip sending a duplicate
          if (!anime.telegramNotified) {
            publishToTelegram({
              type: 'movie',
              title: seriesTitle,
              animeId,
              subOrDub,
              rating: anime.rating || anime.score || ep?.rating || '8.5/10',
              genres: anime.genres || anime.genre || [],
              releaseDate: anime.released || anime.releaseYear || new Date().toISOString().split('T')[0],
              description: anime.description || anime.synopsis || ep?.description || '',
              poster: seriesPoster,
              banner: seriesBanner
            }).catch(() => {});
          }
          ...
          publishToTelegram({
            ...
            title: seriesTitle,
            animeId,
            episodeNumber: firstEpKey,
            subOrDub,
            rating: ep?.rating || anime.rating || '8.5/10',
...
export async function getCustomEpisodes(animeId: string): Promise<Record<string, any>> {
  ...
  const mergedEps = { ...localEps, ...(finalEps || {}) };
  setLocalItem(cacheKey, mergedEps);
  return mergedEps;
}
...
export async function addAdvertisement(id: string, ad: any) {
  ...
  await runWithTimeout(set(adRef, cleanForFirebase(ad)), DB_TIMEOUT_MS, null);
}
