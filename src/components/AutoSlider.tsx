...
export function AutoSlider({ title, animes, direction = 'right' }: AutoSliderProps) {
  ...
  const handleDragMove = (clientX: number, clientY: number = 0, isTouch: boolean = false) => {
    if (!isDraggingRef.current || !trackRef.current) return;
    
    const deltaX = clientX - startXRef.current;
    const deltaY = clientY - startYRef.current;
    
    // On touch devices, if vertical movement dominates early on, treat as page scroll
    if (isTouch) {
  ...
  return (
    <div className="py-6 overflow-hidden relative">
      ...
      <div
        ref={containerRef}
        className="w-full overflow-hidden cursor-grab active:cursor-grabbing select-none touch-action-auto"
        ...
        onTouchMove={(e) => handleDragMove(e.touches[0].clientX, e.touches[0].clientY, true)}
      ...
      >
        ...
        >
          {tripledAnimes.map((anime, index) => (
            <div
               key={`${anime.id}-${index}`}
               className="w-[125px] sm:w-[160px] md:w-[200px] shrink-0"
            >
